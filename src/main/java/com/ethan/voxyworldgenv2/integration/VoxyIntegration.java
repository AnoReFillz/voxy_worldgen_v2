...
public final class VoxyIntegration {
    ...
    private static void initialize() {
        ...
        try {
            ...
            VoxyWorldGenV2.LOGGER.info("voxy integration initialized (enabled: {}, raw: {}, voxyEnabled: {})", enabled,
                    rawIngestMethod != null, voxyEnabledMethod != null);
    ...
    public static void rawIngest(net.minecraft.world.level.Level level,
            net.minecraft.world.level.chunk.LevelChunkSection section, int cx, int cy, int cz,
            net.minecraft.world.level.chunk.DataLayer blockLight, net.minecraft.world.level.chunk.DataLayer skyLight) {
        if (!initialized)
            initialize();
        if (rawIngestMethod == null || worldIdentifierOfMethod == null)
            return;
    ...
    public static boolean isVoxyRenderingEnabled() {
        if (!initialized)
            initialize();
        if (!enabled)
            return true; // voxy not present, don't suppress generation
        if (voxyEnabledMethod == null)
