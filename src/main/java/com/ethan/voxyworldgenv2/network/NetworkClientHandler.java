...
public class NetworkClientHandler {
    ...
    private static void handleLODData(NetworkHandler.LODDataPayload payload) {
        ...
        // discard LOD data from a different dimension to prevent cross-dimension
        // rendering artifacts (issue #43)
        ...
        for (NetworkHandler.LODDataPayload.SectionData sd : payload.sections()) {
            ...
            if (sd.blockLight() != null)
                bytes += sd.blockLight().length;
            if (sd.skyLight() != null)
                bytes += sd.skyLight().length;
        }
        ...
        for (NetworkHandler.LODDataPayload.SectionData sectionData : payload.sections()) {
            ...
            try {
                // recreate section using the biome registry (1.21.1 constructor)
                Registry<Biome> biomeRegistry = level.registryAccess().registryOrThrow(Registries.BIOME);
                LevelChunkSection section = new LevelChunkSection(biomeRegistry);

                // we need to read the states and biomes back using RegistryFriendlyByteBuf for
                // palette consistency
